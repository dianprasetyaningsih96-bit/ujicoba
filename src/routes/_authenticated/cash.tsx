...
function CashPage() {
  ...
  const totals = useMemo(() => {
    ...
    // "Siap Jual" diambil dari cash_balances (balance = SUM per cabang dijumlahkan),
    // yang kini selalu sinkron otomatis via trigger di cash_movements.
    // totalBought & totalSold dari RPC ditampilkan sebagai informasi riwayat.
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      {showValasSummary && (
        <Card>
          ...
          <CardContent>
            <pre className="text-[10px] bg-slate-100 p-2 rounded mb-4 overflow-auto max-h-32">
              DEBUG INFO (Tolong fotokan ini):
              {JSON.stringify(balances?.filter(b => b.currencies?.code === 'AUD'), null, 2)}
            </pre>
            {balances === null ? (
